...
def dual_imaging_session_to_nwb(
    data_dir_path: Union[str, Path], output_dir_path: Union[str, Path], session_id: str, session_date:str, stub_test: bool = False
):
    ...
    # Add 2p Imaging
    imaging_path = data_dir_path / f"{session_id}_2p" 
    source_data.update(dict(TwoPhotonImagingGreenChannel=dict(folder_path=str(imaging_path), file_pattern=f"{session_date}*.tif")))
    ...
    # Add 1p Imaging
    meso_imaging_path = data_dir_path / f"{session_id}_1p" 
    ...
    # metadata["Subject"].update(session_id=session_id)
...
if __name__ == "__main__":
    ...
    stub_test = True
    session_id = "dbvdual035"
    session_date = "20201230"
